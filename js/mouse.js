            ...
            if(current.attack.timer<=0){
                if(current.attack.trigger){
                    ...
                    for(a=0,la=current.combatants[current.stack[0].type].attacks.length;a<la;a++){
                        if(pointInsideBox({position:inputs.rel},{position:{x:50+a*90,y:110},width:80,height:80})&&(current.combatants[current.stack[0].type].uses[a]>0||types.attack[current.combatants[current.stack[0].type].attacks[a]].uses==0)){
